...
function getLastDataScroll(inner) {
  ...
  rows.forEach(row => {
    ...
    // Para cada pozo: la celda 1 es fecha/hora/tapon, la 2 es fecha fin frac
    // Considerar relevante si la celda de tapon tiene un número válido o la fecha/hora no está vacía
    for (let i = 1; i < cells.length; i += 2) {
      const cell = cells[i];
      if (!cell) continue;
      // Buscar tapon
      const taponSpan = cell.querySelector('.tapon-value');
      let taponVal = taponSpan ? taponSpan.textContent.replace('m','').trim() : '';
      // Si es número válido y no vacío
      if (taponVal && !isNaN(Number(taponVal))) {
        targetRow = row;
        continue;
      }
      // Buscar fecha/hora
      const dateDiv = cell.querySelector('.date-part');
      const timeDiv = cell.querySelector('.time-part');
      if ((dateDiv && dateDiv.textContent.trim() !== '') || (timeDiv && timeDiv.textContent.trim() !== '')) {
        targetRow = row;
      }
    }
  });
...
/**
 * Renderiza una vista móvil simplificada: KPIs arriba y lista de pozos.
 * Cada pozo abre una página/ventana interna con su propia tabla de etapas.
 */
function renderMobileView(data) {
  const main = document.getElementById('main');
  if (!main) return;
  main.innerHTML = '';
  // KPIs: mostrar todos los elementos de data.stock y lastUpdate
  const kpiWrap = document.createElement('div');
  kpiWrap.className = 'mobile-kpis';
  try {
    // helper para formatear valores: porcentajes, fechas (excel serial), horas
    function formatKpiValue(name, raw) {
      ...
      if (!isNaN(num)) {
        // porcentaje heurística
        if (lname.includes('avance') || (lname.includes('fractura') && lname.includes('avance')) || lname.includes('%')) {
          const pct = (num >= 0 && num <= 1) ? num * 100 : num;
          return pct.toFixed(1) + '%';
        }
        // fechas numéricas (serial excel)
        if (lname.includes('fecha') || lname.includes('fractura')) {
          const d = excelSerialToDate(num);
          if (d && !isNaN(d.getTime())) return d.toLocaleDateString('es-AR');
        }
        // por defecto número con 2 decimales si no entero
        return Number.isInteger(num) ? String(num) : num.toFixed(2);
      }
      // intentar parsear string fecha/hora dd/mm/yyyy
      const dt = parseDateTimeString(str);
      if (dt) {
        // si incluye hora devolver fecha y hora corta
        const time = dt.toLocaleTimeString('es-AR', { hour12: false });
        return dt.toLocaleDateString('es-AR') + (time ? ' ' + time : '');
      }
      return str;
    }
    ...
    if (Array.isArray(data.stock)) {
      data.stock.forEach(item => {
        ...
        const stockRaw = (item.STOCK === undefined || item.STOCK === null) ? '' : item.STOCK;
        if (!name) return;
        const c = document.createElement('div');
        c.className = 'mobile-kpi';
        const value = formatKpiValue(name, stockRaw);
        c.innerHTML = `<div class="mobile-kpi-title">${name}</div><div class="mobile-kpi-value">${value}</div>`;
    ...
    if (data.lastUpdate) {
      ...
      last.className = 'mobile-kpi';
      const d = new Date(data.lastUpdate);
      last.innerHTML = `<div class="mobile-kpi-title">Actualización</div><div class="mobile-kpi-value">${d.toLocaleString('es-AR', { hour12: false })}</div>`;
      kpiWrap.appendChild(last);
    }
  ...
  list.className = 'mobile-well-list';
  data.wells.forEach((well, idx) => {
    const item = document.createElement('button');
    item.className = 'mobile-well-item';
    item.textContent = well.name;
    item.addEventListener('click', () => renderMobileWellDetail(well, idx));
...
/**
 * Muestra la tabla detallada de un solo pozo en la vista móvil.
 */
function renderMobileWellDetail(well, idx) {
  const main = document.getElementById('main');
  if (!main) return;
  main.innerHTML = '';
  ...
  back.addEventListener('click', () => {
    // reconstruir data actual desde currentData
    if (currentData) renderMobileView(currentData);
  });
  main.appendChild(back);
  const title = document.createElement('h2');
  title.textContent = well.name;
  main.appendChild(title);
  // construir tabla simplificada con 3 columnas: Etapa | Fecha/Hora (HH:MM) | Prof | Fecha Fractura
  const table = document.createElement('table');
  const thead = document.createElement('thead');
  const trh = document.createElement('tr');
  trh.innerHTML = '<th>Etapa</th><th>Hora</th><th>Prof (m)</th><th>Fecha Fractura</th>';
  thead.appendChild(trh);
  table.appendChild(thead);
  const tbody = document.createElement('tbody');
  well.etapas.forEach(et => {
    const tr = document.createElement('tr');
    const tdE = document.createElement('td'); tdE.textContent = et.etapa || ''; tdE.style.textAlign = 'center';
    const tdTime = document.createElement('td');
    let time = '';
    if (et.fechaHora && et.fechaHora.includes(',')) {
      const parts = et.fechaHora.split(',');
      time = parts.slice(1).join(',').trim();
      const m = time.match(/(\d{1,2}:\d{2})/); if (m) time = m[1];
    }
    tdTime.textContent = time;
    const tdProf = document.createElement('td');
    const prof = (et.profundidad === null || et.profundidad === undefined) ? '' : et.profundidad;
    tdProf.textContent = prof !== '' ? String(prof) : '';
    tdProf.style.textAlign = 'center';
    const tdFrac = document.createElement('td');
    tdFrac.textContent = et.fechaFractura || '';
    tdFrac.style.textAlign = 'center';
    tr.appendChild(tdE); tr.appendChild(tdTime); tr.appendChild(tdProf); tr.appendChild(tdFrac);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    // Exponer en window para que otras funciones puedan consultarlo
    window._IS_TV = _IS_TV;
...
function updateDashboard(data) {
  ...
  // Si hay menos de 6 pozos ajustar tamaño del texto aplicando clase
  try {
    if (Array.isArray(data.wells) && data.wells.length < 6) {
      document.body.classList.add('few-wells');
    } else {
      document.body.classList.remove('few-wells');
    }
  } catch (e) {}

  // Detectar si se solicita la vista móvil vía parámetro URL (mobile=1)
  const urlParams = new URLSearchParams(window.location.search);
  const mobileRequested = urlParams.get('mobile') === '1';
  const isTv = Boolean(window._IS_TV);
  // Si se pidió la vista móvil y NO estamos en una TV, renderizar vista móvil
  if (mobileRequested && !isTv) {
...
function renderTables(data) {
  ...
  for (let rowIndex = 0; rowIndex < maxRows; rowIndex++) {
    ...
    data.wells.forEach((well, wellIdx) => {
      ...
      // Obtener color de fondo claro para este pozo
      let bgColor = '';
      try {
        // Usar el color de encabezado, pero con opacidad baja
        let color = (window.wellColors && window.wellColors[wellIdx]) || (window.defaultWellColors && window.defaultWellColors[wellIdx]) || '#e8eef8';
        if (color.startsWith('#')) {
          // Convertir a rgba con opacidad 0.13
          if (color.length === 7) {
            const r = parseInt(color.substr(1,2),16);
            const g = parseInt(color.substr(3,2),16);
            const b = parseInt(color.substr(5,2),16);
            bgColor = `rgba(${r},${g},${b},0.13)`;
          } else {
            bgColor = color;
          }
        } else {
